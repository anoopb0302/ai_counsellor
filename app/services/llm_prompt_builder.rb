class LlmPromptBuilder
  ...
  def build_prompt
    ...
    total_time = Benchmark.realtime do
      ...
      rerank_time = Benchmark.realtime do
        top_chunks = rerank_chunks(initial_chunks)
      end
  ...
  def rerank_chunks(chunks)
    ...
    scored = chunks.map do |chunk|
      ...
      score += 1 if chunk.category.to_s.downcase.in?(query_down)
      score += 1 if chunk.subcategory.to_s.downcase.in?(query_down)
      score += 1 if chunk.tags.any? { |tag| query_down.include?(tag.downcase) }

      # Rails.logger.debug "[RERANK] Title: #{chunk.title}, Score: #{score}"
